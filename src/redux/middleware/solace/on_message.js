...
export default (store) => (msg) => {
    ...
    if (dest.startsWith("TIC")) {
        const [code, date, time, , , , , , close, , , , , volume, volsum, , , , , simtrade] = unpack(msg.getBinaryAttachment())
        // console.log(`tick ${code} ${date} ${time} ${close} ${volume} ${volsum} ${simtrade}`)
        store.dispatch(update_price({ type: "tick", price: close, volume: volume }), "UPDATE_TICK")
        ...
        const [code, date, time, , , bid_price, bid_volume, , ask_price, ask_volume, , , , , , , simtrade] = unpack(msg.getBinaryAttachment())
        ...
        store.dispatch(update_price({ type: "bidask", ask: ask, bid: bid }), "UPDATE_BIDASK")
    }
}